...
import { TR_VOTE_MIN } from '../lib/yerli.js'

const NOW = new Date('2026-08-02T00:00:00Z')

describe('railsFor', () => {
  it('diziler ve filmler için 4\'er raf üretir', () => {
    expect(railsFor('diziler', NOW)).toHaveLength(4)
    expect(railsFor('filmler', NOW)).toHaveLength(4)
  })
  ...
  it('trend ve bilinmeyen sekmelerde raf üretmez', () => {
    ...
    expect(railsFor('trend', NOW)).toEqual([])
    expect(railsFor('sanaozel', NOW)).toEqual([])
  })

  it('her rafın zorunlu alanları vardır', () => {
    for (const tab of ['diziler', 'filmler']) {
      railsFor(tab, NOW).forEach(r => {
  ...
  it('raf anahtarları sekme içinde tekildir', () => {
    for (const tab of ['diziler', 'filmler']) {
      const keys = railsFor(tab, NOW).map(r => r.key)
      expect(new Set(keys).size).toBe(keys.length)
    }
  })

  it('dizi rafları tv, film rafları movie uç noktasını hedefler', () => {
    expect(railsFor('diziler', NOW).every(r => r.mediaType === 'tv')).toBe(true)
    expect(railsFor('filmler', NOW).every(r => r.mediaType === 'movie')).toBe(true)
  })

  it('"Bu Ay Çıkanlar" son 60 günü, "Yeni Vizyon" son 90 günü kapsar', () => {
    const dizi = railsFor('diziler', NOW).find(r => r.key === 'bu-ay')
    expect(dizi.params['first_air_date.gte']).toBe('2026-06-03')

    const film = railsFor('filmler', NOW).find(r => r.key === 'yeni-vizyon')
  ...
  it('tarih parametreleri TMDB\'nin beklediği YYYY-MM-DD biçimindedir', () => {
    const all = [...railsFor('diziler', NOW), ...railsFor('filmler', NOW)]
    for (const r of all) {
  ...
  it('puan bazlı raflar oy eşiği uygular ve istemcide yeniden sıralanır', () => {
    const r = railsFor('diziler', NOW).find(x => x.key === 'yuksek-puanli')
    expect(r.params['vote_count.gte']).toBe(TR_VOTE_MIN)
    expect(r.sortByYerli).toBe(true)
  })
  ...
  it('"Bu Ay Çıkanlar" popülerliğe göre gelir, Yerli Skor ile sıralanmaz', () => {
    ...
    const r = railsFor('diziler', NOW).find(x => x.key === 'bu-ay')
    expect(r.params.sort_by).toBe('popularity.desc')
    expect(r.sortByYerli).toBeUndefined()
  })

  it('platform rafı yalnız TMDB\'de kataloğu olan yerli sağlayıcıları kullanır', () => {
    const r = railsFor('diziler', NOW).find(x => x.key === 'platformda')
  ...
  it('kült rafları gridAction olarak yıl eşiği taşır', () => {
    expect(railsFor('diziler', NOW).find(r => r.key === 'kult').gridAction)
      .toEqual({ type: 'yearsBefore', value: 2015 })
    expect(railsFor('filmler', NOW).find(r => r.key === 'kult').gridAction)
